...
import com.example.demo.todo.TodoData;
import com.example.demo.todo.TodoRep;
import com.example.demo.todo.resultMessage;
...
public class Controller {
    ...
    public ResponseEntity<?> createNewTodo(@RequestBody TodoData todo) {
        ...
        if (todoDataList.isTitleExists(todo.getTitleStat())) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body(new ErrorMessage("Error: TODO with the title [" + todo.getTitleStat() + "] already exists in the system"));
        }
    ...
    @GetMapping("/size")
    public ResponseEntity<?> sizeWithDFilter(@RequestParam (name = "status") String stat) {
        ...
        if(stat.equals("ALL"))
            ...
            integerCounter = Integer.valueOf((int) todoDataList
                    .getAll()
                    .stream()
                    .filter(td -> stat.equals(td.getStatus().toString())).count());
    ...
    public ResponseEntity<?> getTodoData(@RequestParam (name = "status") String status,
        ...
        if(status.equals("ALL"))
            ...
            listDataAfterFilter = todoDataList.getAll()
                    .stream()
                    .filter(td -> status.equals(td.getStatus().toString()))
                    .toList();
        }
        if(howToSort != null) {
            if (howToSort.equals("ID")) {
                listDataAfterFilter = listDataAfterFilter
                        .stream()
                        .sorted(Comparator.comparing(TodoData::getId))
                        .toList();
            } else if (howToSort.equals("DUE_DATE")) {
                listDataAfterFilter = listDataAfterFilter
                        .stream()
                        .sorted(Comparator.comparing(TodoData::getDate))
                        .toList();
            } else {
                listDataAfterFilter = listDataAfterFilter
                        .stream()
                        .sorted(Comparator.comparing(TodoData::getTitleStat, String.CASE_INSENSITIVE_ORDER))
                        .toList();
            }
        }
        else {
        ...
        loggerTodo.debug("There are a total of {} todos in the system. The result holds "
                + "{} todos", todoDataList.getAll().size(), listDataAfterFilter.size());
